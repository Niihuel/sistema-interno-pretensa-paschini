...
export class DailyBackupsService {
  ...
  async getTodayBackup() {
    ...
    if (!backup) {
      ...
      // Crear backup con archivos dinámicos
    ...
    // Actualizar disco si cambió
    if (backup.diskId !== disk.id) {
      backup = await this.prisma.dailyBackup.update({
        where: { id: backup.id },
        data: { diskId: disk.id },
        include: DAILY_BACKUP_INCLUDE,
      });
   ...
   * Genera eventos para TODOS los días del mes, incluso si no existe backup
   */
  async getBackupsAsCalendarEvents(year: number, month: number) {
    ...
    const backupsByDate = new Map(
      backups.map((b) => [startOfDay(b.date).toISOString(), b])
    );

    // Generar eventos para TODOS los días del mes
    const events: any[] = [];
    const currentDate = new Date(start);

    while (currentDate <= end) {
      // Generar eventos para todos los días (incluye fines de semana)
        const dateKey = startOfDay(currentDate).toISOString();
        const backup = backupsByDate.get(dateKey);

        if (backup) {
          // Backup existe: crear evento con datos reales
          events.push({
            id: backup.id,
            title: `Backup ${backup.disk?.name || 'Disco'}`,
            description: backup.notes || `Backup en ${backup.disk?.name || 'disco'}`,
            startTime: backup.date.toISOString(),
            endTime: null,
            allDay: true,
            location: null,
            color: backup.completedAt ? '#10b981' : '#f59e0b',
            ...
            metadata: {
              ...
              completed: backup.completedAt !== null,
              statuses: {
          ...
          // Backup no existe: calcular qué disco correspondería
          const disk = await this.resolveDiskForDate(currentDate);

          if (disk) {
            // Crear evento "virtual" para backup pendiente
            events.push({
              id: `virtual-${format(currentDate, 'yyyy-MM-dd')}`,
              title: `Backup Pendiente (${disk.name})`,
              description: `Backup diario pendiente en ${disk.name}`,
              startTime: currentDate.toISOString(),
              endTime: null,
              allDay: true,
              location: null,
              color: '#6b7280', // Gris para pendientes
              priority: 'NORMAL' as const,
              tags: ['backup', 'sistema', 'pendiente'],
              metadata: {
                type: 'daily-backup',
                backupId: null,
                diskId: disk.id,
